...
class GenBar(QtWidgets.QDialog):
    ...
    def setup_ui(self,subs, vid_name, title, **kwargs):
        ...
        self.threadClass = GenThread(subs, vid_name, title,kwargs["output_dir"], kwargs["limit"], kwargs["dim"], kwargs["note_type"], kwargs["text_field"], kwargs["audio_field"], kwargs["pic_field"])
...
class GenThread(QtCore.QThread):
    ...
    def __init__(self,info, video, vid_title, output_dir, limit, dim, note_type, text_field, audio_field, pic_field):
        self.info = info
        self.video = video
    ...
    def run(self):
        ...
        for sub in info:
            ...
            command = ffmpeg + ' -ss ' + str(
                    startTime.time()) + ' -i ' + '"' + self.video + '" ' + '-s ' + self.dim + ' -vframes 1 -q:v 2 ' + "-loglevel quiet " + '"' + p_name + '"'
